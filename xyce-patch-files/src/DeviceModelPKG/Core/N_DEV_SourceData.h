...
{
 public:
  PWLinDynData(const DeviceEntity  & device, const std::vector<Param> & paramRef,
               const SolverState   & ss1,
               const DeviceOptions & do1);
...
# include "N_DEV_BridgeOp.inc"
  ...
  typedef int (*Callback)(PWLinDynData *,void *ext_data, BridgeOP op, void *op_data); // returns zero if OK
...
private:
  ...
  Callback BindCB(const char *lib_name,const char *fn_name,const char *args,void **data);

public:
  inline int CallBridge(BridgeOP op,void *data = NULL) { return (*BridgeFn)(this,BridgeFnData,op,data); }

  inline tTVVEC *getTVVEC() { return &TVVEC; }

  inline void RecalculateBreaks() { preComputedBreakpointsDone = false; }
  inline void ResetNUM()          { NUM = TVVEC.size(); RecalculateBreaks(); }

  inline int addBreak (double bp) { next_bp = bp; return 0; }

  inline c_string getSrcName()  { return sourceName_.c_str(); }
  inline c_string getTypName()  { return typeName_.c_str(); }
  inline c_string getPrmName()  { return defaultParamName_.c_str(); }
};
